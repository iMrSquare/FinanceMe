...
import type { PersonalCategoria, PersonalBanco } from '@/lib/db';
...
interface Item { id: number; nombre: string; color: string; }

function GestionSection({ title, color, items, apiBase, onRefresh }: {
  ...
  async function addNew() {
    ...
    setSaving(true);
    await fetch(apiBase, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre: newNombre.trim(), color: newColor }) });
  ...
  async function saveEdit() {
    if (!editId) return;
    setSaving(true);
    await fetch(`${apiBase}/${editId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre: editNombre, color: editColor }) });
    setEditId(null); setSaving(false); onRefresh();
  }

  function startEdit(item: Item) { setEditId(item.id); setEditNombre(item.nombre); setEditColor(item.color); }

  function del(item: Item) {
    setConfirm({ msg: `¿Eliminar "${item.nombre}"?`, fn: async () => {
      await fetch(`${apiBase}/${item.id}`, { method: 'DELETE' }); onRefresh();
    }});
  }
