...
import type { RegistroLuz, Categoria } from '@/lib/db';
import { BoltIcon, PencilIcon, TrashIcon, SettingsIcon } from './icons';
...
function KWhBar({ kwh, maxKwh }: { kwh: number | null; maxKwh: number }) {
  if (kwh == null) return null;
  const scale = Math.max(maxKwh, 500);
  const pct = Math.min((kwh / scale) * 100, 100);
  const over = kwh > 500;
  return (
    <div className="rounded-full h-1.5 overflow-hidden" style={{ width: 120, background: 'var(--divider)' }}>
      <div className={`h-1.5 rounded-full transition-all ${over ? 'bg-red-400' : 'bg-yellow-400'}`} style={{ width: `${pct}%` }} />
    </div>
  );
}

const inputCls = 'w-full rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 border';

type ModalState = { type: 'none' } | { type: 'add' } | { type: 'edit'; item: RegistroLuz } | { type: 'gestionar' };
...
export default function RegistroLuzClient({ registros: initRegistros, companias: initCompanias, canEdit = true }: Props) {
  ...
  const byYear = registros.reduce((acc, r) => { (acc[r.anio] ??= []).push(r); return acc; }, {} as Record<number, RegistroLuz[]>);
  ...
  async function save(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      kwh: fd.get('kwh') ? parseFloat(fd.get('kwh') as string) : null,
      ...
      precio_kwh: fd.get('precio_kwh') ? parseFloat(fd.get('precio_kwh') as string) : null,
      compania: fd.get('compania') || null,
    };
    if (modal.type === 'edit') {
      await fetch(`/api/registros/luz/${modal.item.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    } else {
      await fetch('/api/registros/luz', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    }
  ...
  async function del(id: number) {
    setConfirmState({ msg: '¿Eliminar este registro?', fn: async () => {
      await fetch(`/api/registros/luz/${id}`, { method: 'DELETE' });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold" style={{ color: 'var(--text-primary)' }}>Registro Luz</h1>
            <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>Historial de consumo eléctrico</p>
          </div>
          <InfoExpand title="¿Qué es Registro Luz?">
            <p>Apunta aquí manualmente los consumos de luz: primero añade la compañía y después cada lectura o factura para ver un historial del gasto. Estos registros son independientes del Presupuesto y del Mes.</p>
          </InfoExpand>
        </div>
          ...
          {years.length > 0 && (
            <div className="relative">
              <select
                ...
                className="appearance-none bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm rounded-2xl pl-4 pr-8 py-2 cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-400 transition-colors shadow-lg shadow-amber-500/30"
              >
          ...
          {canEdit && (
            <>
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-2xl shadow-lg shadow-amber-500/30 transition-colors"
              >
                + Añadir registro
              </button>
            </>
        ...
        <div key={activeYear} className="glass-card rounded-3xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="theme-table">
              ...
              <tbody>
                {(byYear[activeYear] ?? []).map((r, _idx, arr) => {
                  const maxKwh = Math.max(...arr.map(x => x.kwh ?? 0), 500);
                  return (
                    <tr key={r.id} style={{ cursor: isMobile && canEdit ? 'pointer' : undefined }}
                      ...
                      <td className="py-3 px-4 text-right font-mono" style={{ color: 'var(--text-secondary)' }}>
                        {fmtNum(r.kwh) ?? <span style={{ color: 'var(--text-muted)' }}>—</span>}
                      </td>
                      <td className="py-3 px-4" style={{ width: 130 }}>
                        <KWhBar kwh={r.kwh} maxKwh={maxKwh} />
                      </td>
                      ...
                      <td className="py-3 px-4 text-right font-mono" style={{ color: 'var(--text-secondary)' }}>
                        {r.precio_kwh != null ? `${r.precio_kwh.toLocaleString('es-ES', { minimumFractionDigits: 3, maximumFractionDigits: 4 })} €` : '—'}
                      </td>
          ...
          <div className="glass-card rounded-3xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-5 pb-4" style={{ borderBottom: '1px solid var(--divider)' }}>
              <h3 className="font-bold text-lg flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
                ...
                {editItem ? 'Editar Registro de Luz' : 'Nuevo Registro de Luz'}
              </h3>
            ...
            <form onSubmit={save} className="px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1" style={{ color: 'var(--text-secondary)' }}>KWh</label>
                  <input name="kwh" type="number" step="0.01" defaultValue={editItem?.kwh ?? ''}
                    className={inputCls} style={inputStyle} />
                </div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1" style={{ color: 'var(--text-secondary)' }}>Precio €/KWh</label>
                  <input name="precio_kwh" type="number" step="0.0001" defaultValue={editItem?.precio_kwh ?? ''}
                    className={inputCls} style={inputStyle} />
                </div>
              </div>
              <div>
              ...
              <div className="flex gap-2 justify-end pt-1">
                ...
                <button type="submit" disabled={loading}
                  className="px-4 py-2 text-sm font-bold bg-amber-500 hover:bg-amber-600 text-white rounded-2xl shadow-lg shadow-amber-500/30 disabled:opacity-50 transition-colors">
      ...
      {/* Gestionar compañías modal */}
      {modal.type === 'gestionar' && (
        <GestionarCompaniasModal
          tipo="luz"
          titulo="Compañías de Luz"
          companias={companias}
...
function GestionarCompaniasModal({ tipo, titulo, companias: init, onClose, onChanged }: {
  tipo: string;
  titulo: string;
  companias: Categoria[];
  onClose: () => void;
  ...
  async function addNew() {
    ...
    const updated = [...cats, { id, tipo, nombre: newName.trim(), color: newColor } as Categoria];
    setCats(updated); onChanged(updated);
